...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, ... }:
  {
    ...
    package = {
      specVersion = "1.24";
      identifier = { name = "ghc-mod"; version = "5.9.0.0"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "8.0") (hsPkgs."ghc-boot" or (buildDepError "ghc-boot"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") (hsPkgs."ghc-syb-utils" or (buildDepError "ghc-syb-utils"));
        buildable = true;
        };
      };
    } // rec { src = (pkgs.lib).mkDefault ./submodules/ghc-mod; }